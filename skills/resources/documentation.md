...
*   **Fallback Environment Variables:** The documentation previously implied the Promptfoo configuration supports `EVAL_MODEL` natively, but the recent CI/CD overhaul incorporates an intelligent sequential failover framework inside `run-evals.js` (falling back automatically from `gemini-3.5-flash-lite` to `gemini-3.5-flash` in the event of a `429` rate limit), making duplicate environment variable configurations fully obsolete.
...
*   **`JOURNAL_3.md (aka AGENT.md)` vs `JOURNAL_*.md` Priority Fallback Logic:** The README instructs users to create `JOURNAL_3.md (aka AGENT.md)`. However, `skills/hooks/mabels-grappling-hooks/journal-snatch.js` contains a complex, undocumented fallback cascade. It explicitly checks for `JOURNAL_3.md (aka AGENT.md)` first, but if missing, it recursively scans for, parses, and sequentially concatenates `JOURNAL_1.md`, `JOURNAL_2.md`, and `JOURNAL_3.md` into the context payload. This cascading injection behavior needs deep technical elaboration.
