...
*   **Fallback Environment Variables:** The documentation previously implied the Promptfoo configuration supports `EVAL_MODEL` natively, but the recent CI/CD overhaul dynamically resolves the active model provider directly from `promptfooconfig.yaml` on execution (falling back to `google:gemini-3.5-flash-lite` if unresolved), establishing a robust single source of truth and making duplicate `EVAL_MODEL` variables obsolete.
